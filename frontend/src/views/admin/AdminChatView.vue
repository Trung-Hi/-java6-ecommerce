...
const tryConnect = () => {
    ...
    connectInterval = setInterval(() => {
        ...
        if (status.value === "CONNECTED") {
            ...
            clearInterval(connectInterval);
            connectInterval = null;
            console.warn('WebSocket connection failed after 3 retries');
        }
    }, 250);
    setTimeout(() => {
        if (connectInterval) {
                ...
                <section class="admin-chat-right">
                    ...
                    <template v-else>
                        ...
                        <div class="right-history" ref="scrollRef">
                            ...
                            <template v-else>
                                <div v-for="(m, index) in activeHistory" :key="m.id || (m.createdAt + '_' + m.content + '_' + index)" class="msg" :class="{me: m.senderRole === 'ADMIN'}">
                                    <div class="bubble">
...
<style scoped>
.admin-shell{display:grid;grid-template-columns:340px 1fr;gap:20px}
